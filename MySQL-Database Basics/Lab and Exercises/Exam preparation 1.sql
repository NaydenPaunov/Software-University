...
CREATE TABLE airports (
	...
	town_id INT,
	CONSTRAINT fk_airpots_towns_id FOREIGN KEY (town_id) REFERENCES towns(town_id)
);
...
CREATE TABLE customers (
	...
	gender VARCHAR(1) ,
	home_town_id INT,
	CONSTRAINT fk_customers_towns_id FOREIGN KEY(home_town_id) REFERENCES towns(town_id)
);
...
CREATE TABLE flights(
	...
	airline_id INT,
	CONSTRAINT fk_origin_airport_airports_id FOREIGN KEY(origin_airport_id) REFERENCES airports(airport_id),
	CONSTRAINT fk_destination_airport_airports_id FOREIGN KEY(destination_airport_id) REFERENCES airports(airport_id),
	CONSTRAINT fk_airline_airlines_id FOREIGN KEY(airline_id) REFERENCES airlines(airline_id)
);
...
CREATE TABLE tickets(
	...
	customer_id INT,
	flight_id INT,
	CONSTRAINT fk_customer_customers_id FOREIGN KEY(customer_id) REFERENCES customers(customer_id),
	CONSTRAINT fk_flight_flights_id FOREIGN KEY(flight_id) REFERENCES flights(flight_id)
);
...
SELECT t.ticket_id ,a.airport_name,CONCAT(c.first_name,' ',c.last_name) AS full_name FROM tickets AS t
INNER JOIN flights AS f ON t.flight_id = f.flight_id
...
INNER JOIN flights AS f ON ti.flight_id = f.flight_id /* status departing */
...
t.price ,a.airport_name FROM customers AS c
INNER JOIN tickets AS t ON t.customer_id = c.customer_id
INNER JOIN flights AS f ON t.flight_id = f.flight_id
INNER JOIN airports AS a ON f.destination_airport_id = a.airport_id
WHERE f.`status` = 'Delayed'
ORDER BY t.price DESC LIMIT 3;
...
FROM flights AS f
INNER JOIN airports AS o ON f.origin_airport_id = o.airport_id
INNER JOIN airports AS d ON f.destination_airport_id = d.airport_id
WHERE f.`status` = 'Departing'
ORDER BY f.departure_time ASC , f.flight_id ASC ;
 ...
 SELECT * FROM (SELECT DISTINCT c.customer_id , 
CONCAT(c.first_name,' ',c.last_name) AS full_name,
TIMESTAMPDIFF(YEAR,c.date_of_birth,'2016-12-31') AS age
FROM customers AS c 
INNER JOIN tickets AS t ON t.customer_id = c.customer_id
INNER JOIN flights AS f ON f.flight_id = t.flight_id AND f.`status` = 'Arrived') AS result
WHERE age < 21 
ORDER BY age DESC, customer_id ASC;

/* exr 15 */

SELECT a.airport_id,a.airport_name, COUNT(ticket_id)AS passangers 
FROM airports AS a
...
/* Two new tables needed by next exr */
